...
public class HeapPriorityQueue<E> extends Heap<E> implements PriorityQueue<E> {
	 ...
	 * @param compy
	 *            The Comparator defining the priority of these items.
	 */
	...
	/**
	 * Constructor. Initialize this pq to the keys in the given iterable. The number
	 * of keys in the iterable collection is taken as the capacity of the pq.
	 * 
	 * @param items
	 *            An iterable collection of keys taken as the initial contents of
	 *            the pq.
	 * @param compy
	 *            The Comparator defining the priority of these items.
	 */
	...
	public HeapPriorityQueue(Iterable<E> items, Comparator<E> compy) {
		int size = 0;
		for (Iterator<E> it = items.iterator(); it.hasNext(); it.next())
			size++;
	 ...
	 *            the pq.
	 * @param size
	 *            The number of items, passed in to avoid an extra iteration over
	 *            the items to count them.
	 * @param compy
	 *            The Comparator defining the priority of these items.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Is this pq empty? It is if its heap size is zero.
	 * 
	...
	/**
	 * Is this pq full? It is if its heap size is equal to the array's size.
	 * 
	...
	/**
	 * Return (but do not remove) the maximum element. According to the (max-) heap
	 * property, the maximum element should be at position 0.
	 * 
	...
	/**
	 * Indicate that the priority of a key at a given key has changed, which may
	 * affect the internal storage of the pq.
	 * 
	 * @param key
	 *            The key whose priority has changed.
	 */
	...
	/**
	 * Find the location of a given key. This is the bottleneck in PQ performance,
	 * since it reverts to linear search. OptimizedHeapPriorityQueue should override
	 * this.
	 * 
	 * @param key
	 *            The key whose location to find.
	 * @return The location of that key or -1 if nowhere
	 */
	protected int findKey(E key) {
		int i = -1;
		for (int j = 0; i == -1 && j < heapSize; j++)
			if (internal[j].equals(key))
				i = j;
		return i;
	}
