 ...
 * Heap.java
 * 
 * Abstract class to provide the basic functionality of a heap, to be used, for
 * example, in heapsort or in a priority queue.
 *
 * @author Thomas VanDrunen CSCI 345, Wheaton College Originally for CSCI 245,
 *         Spring 2007 Revised June 2, 2015 and Jan 2, 2018
 */
...
public abstract class Heap<E> {
	 ...
	 * 
	 * @param i
	 *            The index whose parent we want.
	 ...
	 * 
	 * @param i
	 *            The index whose left child we want.
	 ...
	 * 
	 * @param i
	 *            The index whose right child we want.
	...
	/**
	 * Interchange the values at two locations. This method should be used instead
	 * of direct changes to the internal array. This method is to be overridden by
	...
	/**
	 * Enforce the heap property on the subtree rooted at the given index. To be
	 * used when initially building a heap and when fixing up a heap after an item
	 * has been removed.
	 * 
	 * @param i
	 *            The index where we want to make a heap. PRECONDITION: The subtrees
	 *            rooted at the left and right children of i are already heaps.
	...
	/**
	 * Correct a heap in which one value is larger than its ancestors. To be used
	 * with insert() and increaseKey().
	 * 
	 * @param i
	 *            The index of the value that may be larger than its ancestors.
	 *            PRECONDITION: The heap property holds for all values in internal
	 *            before heapSize, except that i may be larger than its ancestors.
	 *            POSTCONDITION: internal bounded by heapSize is heap. (That is to
	 *            say: the heap property holds for all values in internal before
	 *            heapSize).
	 * 
	 */
	...
	/**
	 * Display the state of the heap as an array. The entire array is displayed; a
	 * vertical bar (pipe) indicates the end of the heap.
	 * 
	...
	/**
	 * Validate that the subtrees rooted at the children of i, if any, are valid
	 * heaps.
	 * 
	 * @param i
	 *            The parameter the subtrees rooted at whose children should be
	 *            valid heaps
	 * @return True if the indicated node's children are valid heaps
	 */
	protected boolean isHeapBut(int i) {
		int a = left(i), b = right(i), j = a;
		boolean validHeap = true;
		while (validHeap && left(j) < heapSize) {
			validHeap &= compy.compare(internal[j], internal[left(j)]) >= 0;
			validHeap &= (right(j) == heapSize || compy.compare(internal[j], internal[right(j)]) >= 0);
			j++;
			if (j > b) {
				a = left(a);
				b = right(b);
	...
	protected boolean isHeap() {
		...
		for (int i = 0; validHeap && left(i) < heapSize; i++) {
			...
			validHeap &= (right(i) == heapSize || compy.compare(internal[i], internal[right(i)]) >= 0);
		}
		return validHeap;
	}
